...
**Uwaga:** API zwraca stawki w EUR/km. Całkowitą cenę możesz obliczyć po stronie klienta mnożąc stawkę przez dystans trasy.
...
def get_route_pricing(start_code, end_code):
    headers = {
        'X-API-Key': API_KEY,
        'Content-Type': 'application/json'
    }
...
# Użycie
result = get_route_pricing("PL20", "DE49")
...
async function getRoutePricing(startCode, endCode) {
  try {
...
// Użycie
getRoutePricing('PL20', 'DE49').then(result => {
  if (result?.success) {
...
## 🎯 Fuzzy Matching dla Tras Historycznych (v2.4.0+)

### Opis

API automatycznie znajduje najbliższą trasę historyczną, gdy nie ma dokładnego dopasowania kodów pocztowych.

### Algorytm

1. **Próba dokładnego dopasowania** - jeśli istnieją dane dla podanej trasy, są zwracane
2. **Fuzzy matching** - jeśli brak dokładnego dopasowania:
   - Znajduje najbliższy punkt startowy (< 100 km)
   - Dla tego punktu, znajduje najbliższy punkt końcowy (< 100 km)
   - Zwraca statystyki dla dopasowanej trasy

### Poziomy Dokładności

| Accuracy | Opis | Odległości |
|----------|------|------------|
| `exact` | Dokładne dopasowanie | Start < 1 km, End < 1 km |
| `high` | Bardzo podobna trasa | Start < 50 km, End < 50 km |
| `medium` | Podobna trasa | Start < 100 km, End < 100 km |
| `low` | Częściowe dopasowanie | Start < 100 km, End > 100 km |

### Match Info w Odpowiedzi
...
switch(matchInfo.accuracy) {
  case 'exact':
    // Wyświetl normalnie bez ostrzeżenia
    break;
  
  case 'high':
  case 'medium':
    // Pokaż ikonę info z tooltipem
    showInfoIcon(`Dane z podobnej trasy: ${matchInfo.matched_start} → ${matchInfo.matched_end}
                  (±${matchInfo.start_distance_km}km, ±${matchInfo.end_distance_km}km)`);
    break;
  
  case 'low':
    // Pokaż ostrzeżenie
    showWarning(`Uwaga: Dane z trasy o podobnym punkcie startowym, 
                 ale innym końcowym (+${matchInfo.end_distance_km}km)`);
    break;
}
```
...
Zobacz szczegółowy opis algorytmu w pliku `FUZZY_MATCHING_STRATEGY.md`.
...
**v2.4.0** (2024-12-11)
- 🎯 **Fuzzy Matching** - inteligentne dopasowanie tras historycznych (±100km)
- 📍 Cztery poziomy dokładności dopasowania (exact, high, medium, low)
- 🗺️ Wykorzystanie tabeli PostalCodeCoordinates z geocodowanymi kodami
- 📊 Rozszerzona odpowiedź API o `match_info` z metadanymi dopasowania
- 📚 Pełna dokumentacja strategii w FUZZY_MATCHING_STRATEGY.md
